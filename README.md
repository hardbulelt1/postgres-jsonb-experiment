...
3. Посмотрим стратегию TOAST
   ...
   Описание полей из https://postgrespro.ru/docs/postgresql/12/storage-toast:
...
С данными типами полей и искуственными данными реляционная таблица имеет преимущество в скорости обновления и скорости вставки. Но время все равно очень маленькое на базе без нагрузки. При обновлении таблиц, таблица с jsonb увеличивается сильнее. Возможно с другими данными, будет другой результат. Если реляционная таблица будет занимать больше места и поля типа extended после сжатия попадут в toast, то время обновления/вставки увеличится.
